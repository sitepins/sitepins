...
See the [root README](../README.md) for the project overview, self-hosting quickstart, and Docker. GitHub/GitLab app setup lives in [app/README.md](../app/README.md) — this service never touches those credentials.
...
| `S3_ENDPOINT` / `S3_REGION` / `S3_ACCESS_KEY` / `S3_SECRET_KEY` / `S3_BUCKET_NAME` | ✓ | S3-compatible media storage (AWS S3, Cloudflare R2, MinIO, Backblaze B2, DigitalOcean Spaces) — see [Media storage](#media-storage-s3-compatible) |
