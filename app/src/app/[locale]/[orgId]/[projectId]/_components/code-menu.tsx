...
const CodeMenu = ({
  ...
  // Code tree paths are repo-relative (e.g. "content/assets/css").
  // Strip leading "content/" so the final URL becomes /code/assets/css.
  const stripCodePrefix = (filePath: string) =>
    normalizePath(filePath).replace(/^content\//, "");

  const resolveHref = (filePath: string) =>
    `/org-${orgId}/${projectId}/code/${stripCodePrefix(filePath)}`;
