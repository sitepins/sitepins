...
const comboboxItemVariants = cva(
  "relative mx-1 flex h-[28px] items-center rounded-sm px-2 text-sm text-foreground outline-none select-none [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    ...
    variants: {
      interactive: {
        false: "",
        true: "cursor-pointer transition-colors hover:bg-primary hover:text-primary-foreground data-[active-item=true]:bg-primary data-[active-item=true]:text-primary-foreground",
      },
    },
  },
);
...
function InlineComboboxGroup({
  ...
  return (
    <ComboboxGroup
      {...props}
      className={cn(
        "not-last:border-border hidden py-1.5 not-last:border-b [&:has([role=option])]:block",
        className,
      )}
    />
  );
