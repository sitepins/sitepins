...
export const TableElement = withHOC(
  ...
  function TableElement({
    ...
    const hasInitializedColSizesRef = React.useRef(false);

    React.useEffect(() => {
      if (hasInitializedColSizesRef.current) return;
      if (containerWidth <= 0) return;

      const hasStoredSizes = colSizes.some((size) => size > 0);

      if (hasStoredSizes) {
        hasInitializedColSizesRef.current = true;
        return;
      }
      ...
      const availableWidth = containerWidth - controlColumnWidth;
      const colWidth = Math.floor(availableWidth / columnCount);

      for (let i = 0; i < columnCount; i++) {
        setTableColSize(
          editor,
          { colIndex: i, width: colWidth },
          { at: tablePath },
        );
      }
    }, [
    ...
    const resolvedColSizes = React.useMemo(() => {
      ...
      if (containerWidth > 0 && columnCount > 0) {
        ...
        return Array.from({ length: columnCount }, () => colWidth);
      }
    ...
    }, [colSizes, containerWidth, controlColumnWidth, props.element]);
    ...
    const content = (
      ...
      >
        <TableResizeContext.Provider value={resizeController}>
          ...
          >
            <div
              className="bg-ring/70 pointer-events-none absolute inset-y-0 z-36 hidden w-0.75 translate-x-[-1.5px]"
              contentEditable={false}
              ref={dragIndicatorRef}
            />
            <div
              className="bg-ring/80 pointer-events-none absolute inset-y-0 z-35 hidden w-0.75 translate-x-[-1.5px]"
              contentEditable={false}
...
const TableCellResizeControls = React.memo(function TableCellResizeControls({
  ...
  return (
    ...
    >
      <div
        className="pointer-events-auto absolute -top-2 -right-1 z-40 h-[calc(100%+8px)] w-2 cursor-col-resize touch-none"
        onPointerDown={(event) => {
          startResize(event, {
            colIndex,
            direction: "right",
